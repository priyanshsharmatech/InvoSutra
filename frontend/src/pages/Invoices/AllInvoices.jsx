...
const AllInvoices = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(
        API_PATHS.INVOICE.DELETE_INVOICE(invoiceToDelete._id)
      );
  ...
  const filteredInvoices = useMemo(() => {
    return invoices
      .filter(
        // Filter by 'Paid' or 'Unpaid', 'All' shows everything
          ...
          <div className="bg-white rounded-xl shadow-lg w-full max-w-sm p-6">
            ...
            <p className="text-slate-600 text-sm mb-6">
              ...
              <span className="font-medium text-slate-900">
                Invoice #{invoiceToDelete?.invoiceNumber}
              </span>
        ...
        <div className="p-4 sm:p-6 border-b border-slate-200">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-shrink-0">
              <select
                className="w-full sm:w-auto h-10 px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
                value={statusFilter}
                  ...
                  <tr key={invoice._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          // Only 'Paid' or 'Unpaid' statuses now
                          invoice.status === "Paid"
                            ? "bg-emerald-100 text-emerald-800"
                            : "bg-red-100 text-red-800" // Assume anything not 'Paid' is 'Unpaid'
                        }`}
                      >
                        {invoice.status}
                      </span>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      ...
                      >
                        ...
                        {/* Reminder button only for unpaid invoices now */}
                        {invoice.status !== "Paid" && (
                          <Button
                            size="small"
                            variant="ghost"
